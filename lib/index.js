...
class AnyList extends EventEmitter {
   ...
   */
	async getMealPlanningCalendarEvents() {
		...
		this.calendarId = decoded.mealPlanningCalendarResponse.calendarId;
	...
	/**
   * Factory function to create new Meal Planning Calendar Event.
   * @param {object} calendarEvent new calendar event options
   * @return {Promise<MealPlanningCalendarEvent>} calendar event
   */
	async createMealPlanningCalendarEvent(calendarEvent) {
		if (!this.calendarId) {
			await this.getMealPlanningCalendarEvents();
		}

		return new MealPlanningCalendarEvent(calendarEvent, this);
	}
